...
from src.ingest.gemini_author_details import (
    build_batch_request_line,
    parse_batch_result_line,
)
from src.ingest.repo import GeminiAuthorDetailsRow, Repo
from src.utils.io import connect_sqlite
from src.utils.logging import get_logger

logger = get_logger("fetch_gemini_author_details")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=int,
        default=1000,
        help="Max number of distinct NYT authors to include in a new Gemini batch.",
    )
    p.add_argument(
        "--refresh-all",
        action="store_true",
        help="Reprocess all distinct NYT authors, not only missing Gemini author detail rows.",
    )
    p.add_argument(
        "--jsonl-path",
        type=Path,
...
def main() -> None:
    ...
    if not settings.gemini_api_key:
        raise SystemExit("GEMINI_API_KEY is required for Gemini author details.")
    ...
    repo = Repo(conn)
    repo.init_schema()
    author_inputs = repo.list_gemini_author_details_inputs(
        limit=args.limit,
    ...
    if not author_inputs:
        logger.info("No NYT author rows to process.")
        return

    timestamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    jsonl_path = (args.jsonl_path or (gemini_dir / f"gemini_author_details_batch_requests_{timestamp}.jsonl")).resolve()
    manifest_path = resolve_manifest_path(args.manifest_path, gemini_dir, create_new=True, timestamp=timestamp)

    write_jsonl_input(jsonl_path, author_inputs)
    logger.info("Wrote Gemini author details batch input JSONL: %s (requests=%s)", jsonl_path, len(author_inputs))
    ...
    batch_payload = gemini.create_batch_job(
        ...
        display_name=f"gemini-author-details-{timestamp}",
    )
    ...
    manifest = {
        ...
        "request_count": len(author_inputs),
    }
    write_manifest(manifest_path, manifest)

    logger.info("Created Gemini author details batch job: %s", batch_name)
    logger.info("Saved Gemini author details batch manifest: %s", manifest_path)
...
def process_existing_batch(
    ...
    logger.info("Gemini author details batch state: %s (%s)", batch_state, batch_name)
    ...
    if batch_state not in SUCCEEDED_BATCH_STATES:
        if batch_state in TERMINAL_BATCH_STATES:
            ...
            logger.info("Gemini author details batch ended in state %s | error=%s", batch_state, batch_error)
        else:
    ...
    result_path = gemini_dir / f"{sanitize_name(batch_name)}_author_details_results.jsonl"
    result_bytes = gemini.download_file_bytes(result_file_name)
    result_path.write_bytes(result_bytes)
    logger.info("Downloaded Gemini author details batch results: %s", result_path)
...
def write_jsonl_input(jsonl_path: Path, author_inputs: list[Any]) -> None:
    jsonl_path.parent.mkdir(parents=True, exist_ok=True)
    with jsonl_path.open("w", encoding="utf-8") as fh:
        for row in author_inputs:
...
def import_result_jsonl(result_path: Path) -> None:
    ...
    if not raw_lines:
        logger.info("Gemini author details result JSONL was empty: %s", result_path)
        return
    ...
    rows: list[GeminiAuthorDetailsRow] = []
    failures = 0
    for line, parsed in zip(raw_lines, parsed_lines):
        raw_key = extract_line_key(parsed)
        if not raw_key:
            failures += 1
            logger.info("Skipping Gemini author details result line without key: %s", line[:200])
            continue
        detail_row = parse_batch_result_line(line)
        if detail_row.nyt_author_id <= 0:
            failures += 1
            logger.info("Skipping Gemini author details result line with invalid nyt_author_id: %s", raw_key)
            continue
        if detail_row.last_error:
            failures += 1
        rows.append(detail_row)

    upserts = repo.upsert_gemini_author_details(rows)
    conn.close()

    logger.info(
        "Imported Gemini author details batch results: rows=%s upserts=%s failures=%s",
        len(rows),
        upserts,
        failures,
    )
...
def resolve_manifest_path(
    ...
    return (gemini_dir / f"gemini_author_details_batch_manifest_{timestamp}.json").resolve()
